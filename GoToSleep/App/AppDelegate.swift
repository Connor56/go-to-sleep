...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
  ...
  func menuNeedsUpdate(_ menu: NSMenu) {
    ...
    menu.addItem(
      NSMenuItem(title: "Test Overlay", action: #selector(testOverlayClicked), keyEquivalent: ""))
    menu.addItem(
      NSMenuItem(title: "Settings...", action: #selector(settingsClicked), keyEquivalent: ","))
  ...
  private func statusText() -> String {
    ...
    if TimeCheck.isWithinBedtimeWindow(
  ...
  func registerDaemon() {
    ...
    let data = try? PropertyListSerialization.data(
      fromPropertyList: plistContent, format: .xml, options: 0)
  ...
  func unregisterDaemon() {
    ...
    do {
      ...
      process.waitUntilExit()
      print(
        "\(debugMarker) unregisterDaemon launchctl unload exit code: \(process.terminationStatus)")
    } catch {
